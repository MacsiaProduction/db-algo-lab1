...
Около 40% процессорного времени уходит на фазы G1 . Источник давления — `HashMap` и `HashSet`, создаваемые внутри `findNear` на каждый вызов для сбора и дедупликации кандидатов из 8 бакетов полос.
