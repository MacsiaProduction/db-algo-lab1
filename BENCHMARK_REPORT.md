...
**Окружение:** macOS Darwin 25.4.0 · JDK 23.0.2 · `@Fork(1)`
**JMH:** обычно `@Warmup(2×2s)` + `@Measurement(3×2s)`; для `PerfectHashBenchmark.benchBuild` — `2×1s` + `3×1s`
**Профилирование:** async-profiler 4.0 · JFR + collapsed stacks -> HTML / JSON / SVG flamegraphs
**Размеры данных:** N = 1 000 / 100 000 / 1 000 000

## Что изменено в методике

- Убрано создание строк на горячем пути в JMH-бенчмарках.
- Для `HashTableBenchmark.benchUpdate` update-значение теперь той же длины, что и исходное. Старый вариант искусственно провоцировал `truncate()` и завышал цену update.
- Flamegraphs теперь строятся из настоящих collapsed stacks async-profiler, а не из парсинга вывода JMH.
...
- `get` почти плоский по всем размерам. Разница `1K` vs `100K` слишком мала, чтобы считать её отдельным эффектом: обе точки живут в одном page-cache режиме.
- После фикса benchmark-артефакта `update` больше не прыгает в десятки микросекунд и остаётся однозначно в single-digit µs даже на `1M`.
...
- `get` и `update` имеют умеренные хвосты даже на `1M`.
- `insert` резко тяжелее по хвостам, потому что на больших размерах чаще попадает в split и directory update.
- `delete` остаётся худшим steady-state write-path, потому что почти всегда реально меняет размер bucket-файла.
...
- После выравнивания длины update-значения `update` больше не доминируется `ftruncate()`. Теперь это обычная смесь `fstat` + `pread` + `pwrite` + lookup по channel cache.
- `delete` по-прежнему почти полностью syscall-bound. Тут `ftruncate()` уже не артефакт, а реальная цена операции.
- Для `insert` основные потери идут в `pwrite` и работу со split-файлами (`__unlink` / `__open`).
...
- `100K` действительно быстрее `1K`, и это ожидаемо: при `N < fullScanThreshold = 4000` код уходит в полный перебор. То есть `1K` меряет не "LSH уже работает", а fallback-path.
...
- Профиль теперь почти целиком JDK/container-bound, а не GC-bound: `HashSet.iterator` 32.6%, `HashMap.putVal` 15.5%, `HashMap$HashIterator.nextNode` 13.3%, `HashMap$Node.<init>` 7.2%, `TimSort` ~5%.
- GC занимает около `1%`, так что старое объяснение про "LSH упирается в GC" больше не подтверждается обновлёнными данными.
...
- Lookup остаётся самым быстрым из всех трёх структур: даже на `1M` это около `154 ns/op`.
...
Отношение slots/entry стабилизируется около `4.0`, что заметно ниже теоретического потолка `6N`.
...
- Здесь почти нет ни GC, ни I/O. Основная цена — два FNV-1a хеша и преобразование `String -> bytes`.
...
- `PerfectHashMap` остаётся лучшим вариантом для максимально быстрого точечного lookup.
- `ExtendibleHashTable` хорош для персистентного доступа, но write-heavy сценарии всё ещё ограничены системными вызовами.
- Для LSH наиболее "здоровый" режим в этих параметрах — `100K`; на `1M` стоимость уходит в обработку большого множества кандидатов.